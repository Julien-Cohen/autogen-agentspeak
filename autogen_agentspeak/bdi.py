# inspired from https://github.com/javipalanca/spade_bdi/blob/master/spade_bdi/bdi.py
import asyncio
import collections
...
from pseudo_spade.behaviour import CyclicBehaviour
from pseudo_spade.message import Message
...
class BDIAgent(RoutedAgent):
    ...
    def __init__(self, descr:str, asl:str):
        ...
        self.bdi = None # FIXME : ???
        self.bdi_agent = None # FIXME : ???

        super().__init__(descr)

        self.bdi_env = asp_runtime.Environment() # create a new AgentSpeak environment (?)
        self.bdi_actions = asp.Actions(asp_stdlib.actions) # create an empty list of AgentSpeak actions (?)
        #self.bdi.add_actions() # self.bdi initialized by add_behavior (deleted here)
